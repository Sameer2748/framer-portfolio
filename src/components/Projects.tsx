            ...
            <div className={`grid grid-cols-1 ${grid === 2 ? 'md:grid-cols-2' : 'md:grid-cols-3'} gap-4 py-4`}>
                {
                    Projects.map((item, idx) => (
                        <Link href={item.href} key={idx}>
                            <motion.div
                                className="group relative flex flex-col justify-between h-full bg-white dark:bg-neutral-900 rounded-xl shadow  transition duration-200 hover:shadow-lg"
                            ...
                            >
                                ...
                                <div className="mt-4 px-4 pb-2">
                                    <h2 className="text-black dark:text-white font-medium tracking-tight z-20">{item.title}</h2>
                                    <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">{item.description.slice(0,100)}...</p>
                                </div>
                            </motion.div>
